...
% Add in something about document-centric perspective
...
assignments~\citep{} or compute variational parameters~\citep{}.
...
similar documents (Chapter~\ref{}), displaying documents to a user
(Chapter~\ref{}), or connecting documents across languages
(Chapter~\ref{}).
...
learned in making topic models interpretable (Section~\ref{}) could
...
representation models~\cite{ranganath-15}.  Topic models could help
...
deep-learning representation learning could peacefully co-exist for
some time.  Topic models offer advantages of speed and
interpretability, while representation learning can do better for
prediction-based tasks.  They would well become tools that many text
...
rather to make them more accessible.  As we've described, topic models
...
This requires making the tools more usable: the preprocessing often
required of topic models is not straightforward: should we remove
...
possible models should we use, what inference technique gives us the
...
applications.  For further information, we'd encourage the reader to
