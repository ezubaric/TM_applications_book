...
\jbgcomment{I think we can combine the definition of terms a little
  bit with the outline to make it clearer.  Perhaps something like:

Machine translation is X.  

Machine translation assumes data that looks like Y.
However, most collections do not have this property (you have to look
hard to find it; most are created).

But we should be able to use data that are less structured.  Topic
models unlocks comparable and mixed data.  They are \dots

In this chapter, we first discuss how topic models unlocks X data for
applications like (discussed in Chapter X).  We then show how these
models can help machine translation, the usual application of
multilingual NLP. \yhcomment{Added the following paragraph, but not sure
whether it is a good fix.}
}
...
\jbgcomment{Instead of just saying that this is weaker, would probably
be a stronger section if you talk about the applications of this
approach.  For example, the end goal could be a user who could browse
a collection with only monolingual knowledge to find relevant
documents, send them to a bilingual expert for further analysis. \yhcomment{added.}}
...
\jbgcomment{Would be good to have example of what these other
  languages are  \yhcomment{fixed}}
...
\jbgcomment{Need a better transition from previous section.  Perhaps
  something like:
...
However, they require active support of those creating the
...
dictionary.  This section discusses how we can use tools like
dictionaries and orthographic relations between words to help users
...
For instance, tree-based topic models \abr{tlda}~\citep{boyd-graber-07,andrzejewski-09,hu-14:itm}
