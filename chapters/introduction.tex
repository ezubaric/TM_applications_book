...
built upon systematizing, solving, and evaluating this problem.  Google's search service
...
\emph{extended} it.  \abr{lda} is not just widely used, but it is also
widely modified.  Because of these prolific modifcations, we focus on the mechanics of \abr{lda}, which many
...
\index{topic detection and tracking}
...
\index{Dirichlet distribution!parameter}
...
\index{latent Dirichlet allocation}
We now have all the tools we need to tell the complete story of the most popular topic model: latent Dirichlet allocation~\citep[\abr{lda}]{blei-03}.  Latent
...
\index{latent Dirichlet allocation!generative process}
...
topic assignment, the
same word might be assigned to \emph{different} topics in the \emph{same} document.
To estimate \emph{global} properties of the topic model we use aggregate statistics derived from token-level topic assignments.
...
its popularity.  We define $\dc{d}{i}$ as the number of times
document~$d$ uses topic~$i$.  This is larger for more popular topics;
however, it is not a probability because it is larger than one.  We make it a
...
\index{Gibbs sampling!for \abr{lda}}
...
\index{Dirichlet distribution!role in \abr{lda}}
...
\index{latent Dirichlet allocation!implementations}
