...
Making sense of one of these piles requires actually reading the
...
discuss here, but it will later (e.g., Chapter~\ref{sec:css}).  We do
...
combinations of discrete words.\footnote{An emerging trend in natural language
  ...
  topic modeling in Chapter~\ref{ch:conc}.}   Thus, we need a distribution
over discrete sets.
...
\begin{figure}
  ...
  \caption{Given different Dirichlet parameters, the Dirichlet
    ...
    only a small number of elements but don't care which ones.  This
...
% TODO: add in generative example
