...
Throughout this survey, we have been referring to topics with labels such as
...
Despite the fact that this type of reference corpus may cover different domains than the specific training collection, the approach is still effective.
...
between the words is the normalized pointwise mutual information \abr{npmi} computed on a reference corpus, a function that measures whether two words occur together more often than we would expect by chance.  The
...
Topic models are rarely perfect, and quality can vary within a model. Even in good models we often find several poorly fit or improperly combined topics. \citet{chang-09b} shows that
held-out likelihood, a traditional measure of probabilistic model quality,
emphasizes \emph{complexity} rather than the ease of interpretability that users are looking for.  Automated techniques that produce numeric results may not be able to tell you
whether a topic model is good or not, which parts of a model are reliable, or why.\footnote{Automated
