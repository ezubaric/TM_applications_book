...
\cite{Song-2010} apply topic models to model users' preference from users' search history. The idea is very similar to smoothing query language models $p(w|q)$ by topic models, as explained in Equation~\ref{eq:query_word_prob}. The estimation of $p(w|q)$ is split into two parts $p(w|k)$ and $p(k|q)$.
...
\begin{figure}
  ...
  \end{subfigure}
  \end{center}
  \caption{The proposed graphical model for personalized retreival in Figure~\ref{fig:ptm-original} and the actual simplified model used for parameter estimation in Figure~\ref{fig:ptm-simplified} (both figures are from \cite{Harvey-2013}.)}
  \label{fig:ptm}
\end{figure}
...
&\propto p(d) \prod_{w \in q} \sum_k p(w|k) p(u|k) p(k|d) \notag
\end{align}
