...
\jbgcomment{It would be good to contrast a little bit with the role
  that topic models serve in exploring document collections (discussed
  in the intro); i.e., you don't know what you're looking for.  This
  chapter focuses instead on when you do know what you're looking for
  (in the form of a query).  We may even want to call this
  ``traditional information retrieval'' or ``query-based
  IR''. \yhcomment{connected a little.}}

Users start with their information need in the form of queries, and early IR systems treat both the queries and documents as ``bags of words'', retrieve and rank the documents by measuring the word overlap between queries and documents.
...
such semantic relationships. Besides, humans would also use background
...
all topics, provides semantic relations between query words and
...
the ranking results of information retrieval. 
...
probability of generating the given query $q$ can be computed by,
\begin{align}
p(q|d) = \prod_{w \in q} p(w|d) = \prod_{w \in q} \frac{n_{d,w}}{n_{d,\cdot}}
\end{align}

Then the documents are ranked based on this probability $p(q|d)$. Higher
...
of generating the whole query given this document is zero, which may cause ranking problems
for this document.
...
unseen words,
\begin{align}
p(w|d) = \frac{n_{d,w} + \beta p(w|\mathcal{C})}{\sum_{v \in V} n_{d,v} + \beta}
\end{align}
...
\begin{align}
(\beta p(v_1 | \mathcal{C}), \beta p(v_2 | \mathcal{C}), \cdots, \beta p(v_n | \mathcal{C}))
\label{eq:lm-bs}
\end{align}
...
\cite{hofmann-99a} first introduces pLSA to 
...
\begin{align}
p(w|d) = \lambda' \Big( (1 - \lambda) p_{\texttt{ml}}(w|d) + \lambda
p(w|\mathcal{C}) \Big) + (1 - \lambda') p_{\texttt{TM}}(w|d)
\end{align}
...
\begin{align}
p(w|d) = \lambda \frac{n_{d,w} + \beta p(w|\mathcal{C})}{\sum_{v \in V} n_{d,v} + \beta}  + (1 - \lambda) p_{\texttt{lda}}(w|d)
\end{align}
...
the combined query model $\hat{\theta}_{Q'}$ can be computed by,
\begin{align}
\label{eq:qlm-comb}
...
by a direct spelling matching. As a result, topic models have been
...
%\end{align}
\begin{align}
p(w_x|w_y, \alpha) &= \sum_k p(w_x, k | w_y, \alpha) \\
&= \sum_k p(w_x | k, w_y, \alpha) p(k | w_y, \alpha) \notag\\
&= \sum_k p(w_x|k, \alpha) p(k|w_y, \alpha) \notag
\end{align}
where $\alpha$ is the concentration parameter of the Dirichlet prior for document-topic distributions and $p(w_x|k, \alpha)$ is the topic-word distribution which can be learned from topic models, and $p(k|w_y, \alpha)$ can be computed as,
%\begin{align}
...
%\end{align}
\begin{align}
p(k|w_y, \alpha) &= \frac{p(w_y|k,\alpha)p(k|\alpha)}{p(w_y|\alpha)} \\
%&= \frac{p(w_y|k,\alpha)p(k|\alpha)}{\sum_{k'} p(w_y, k'|\alpha)} \notag\\
&= \frac{p(w_y|k,\alpha)p(k|\alpha)}{\sum_{k'} p(w_y|k',\alpha)p(k'|\alpha)} \notag
\end{align}
where $p(w_y|k,\alpha)$ is the topic-word distributions. \cite{Park-2009} also show that $p(k|\alpha) = \frac{\alpha_k}{\sum_j \alpha_j}$.
%\begin{align}
...
%\end{align}
\begin{align}
p(k|w_y, \alpha) &= \frac{p(w_y|k,\alpha) \frac{\alpha_k}{\sum_j \alpha_j} }{\sum_{k'} p(w_y|k',\alpha) \frac{\alpha_{k'}}{\sum_j \alpha_j}} \\
&= \frac{p(w_y|k,\alpha) \alpha_k}{\sum_{k'} p(w_y|k',\alpha) \alpha_{k'}} \notag
\end{align}
...
users may prefer different results even the input queries are the same~\cite{Jansen-2000,Dou-2007}.
In order to meet different users' information need, it is important to understand the user's preference, context, 
etc., and adapt the ranking results accordingly~\cite{Pitkow-2002,Micarelli-2007}. 
This is referred as personalized search, or search personalization. 

There are multiple different ways to do search personalization, two of major directions are contextualization and individualization\cite{Pitkow-2002}. The former is to consider users' conditions in a search activity, for example, time and location. 
The latter focuses more on users' individual characteristics and activities, which is also described as the users' profile. 

\cite{Dou-2007} investigate several heuristics to create user profiles and generate personalized search. 


\cite{Song-2010} apply topic models to model users' preference from users' search history. The idea is very similar to smoothing query language models $p(w|q)$ by topic models, as explained in Equation~\ref{eq:query_word_prob}. 
...
\begin{align}
p(k|q) = \frac{p(k, q)}{\sum_k p(k, q)} \approx \frac{\text{sim}(\theta_k, \theta_q)}{\sum_k \text{sim}(\theta_k, \theta_q)}
\end{align}
...
\cite{Carman-2010} also investigate topic models on large query logs for search personalization and propose a personalization topic model as shown in Figure. 

Instead of just two observed variable ( document $d_i$ and word $w_i$) as in normal topic models, this model adds one extra variable $u_i$ for user and directly estimate the probability of $p(k|w_i, d_i, u_i)$. They further assume the conditional independence among the word $w_i$, the user $u_i$ and document $d_i$ given the topic $k$, and the model can be simplified as,
\begin{align}
p(k|w_i, d_i, u_i) = \frac{p(k,w_i, u_i | d_i)}{p(w_i, u_i | d_i)} \propto p(w_i|k) p(u_i|k) p (k|d_i)
\end{align}

Given the topic distribution of the document, some number of 
